...
class Zend_Cache_Backend_File extends Zend_Cache_Backend implements Zend_Cache_Backend_ExtendedInterface
{
    ...
    {
        throw new \RuntimeException(__CLASS__ . ' should not be used anymore!');
     ...
     *
     * @param string $id cache id
     * @return int | null (a cache is not available) or "last modified" timestamp (int) of the available cache record
     */
    ...
    protected function _loadMetadatas($id)
    {
        ...
        try {
            $tmp = json_decode($result, true, 512, JSON_THROW_ON_ERROR);
    ...
    {
        $file = $this->_metadatasFile($id);
        $result = $this->_filePutContents($file, json_encode($metadatas, JSON_THROW_ON_ERROR));
        if (!$result) {
            return false;
        }
        return true;
     ...
     * @param string $id Cache id
     * @param boolean $doNotTestCacheValidity If set to true, the cache validity won't be tested
     * @return null|int (a cache is not available) or "last modified" timestamp (int) of the available cache record
     */
    protected function _test($id, $doNotTestCacheValidity): ?int
    {
        $metadatas = $this->_getMetadatas($id);
        if (!$metadatas) {
            return null;
